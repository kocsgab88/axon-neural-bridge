"""
    A  X  O  N  |  T E L E G R A M  B R I D G E  v8.4
    ────────────────────────────────────────────────
    ...
    v8.3 → Pipeline cost tracker
           + api_costs tábla: pipeline oszlop hozzáadva
           + /stats: GROUP BY pipeline bontás ikonokkal (⚙️📋✍️📊)
    v8.4 → Quick Wins + History perzisztencia
           + Hardcoded path-ok kiváltva: Path(__file__).parent alapú (GitHub-ready)
           + History SQLite perzisztencia: bot restart után visszatölthető
           + restore_history() /start parancsnál
           + Napi history cleanup JobQueue-ban (7 nap retention)
           + axon_context.py CONTEXT_VERSION → 8
"""
...
from axon_memory import (init_db, save_training_sample, get_stats, format_stats_message,
                         ...
                         # v8.4 history perzisztencia
                         restore_history, persist_history, cleanup_old_history)
...
# ═══════════════════════════════════════════════════════════════
#  ELÉRÉSI UTAK – v8.4: Path(__file__).parent alapú (GitHub-ready)
#  Korábban hardcoded C:\AXON_OPS\AxonV2\ – most hordozható
# ═══════════════════════════════════════════════════════════════
...
# .env betöltés – a bot mappájához relatív
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # v8.4: History visszatöltése bot restart után
    chat_id = str(update.effective_chat.id)
    restored = restore_history(chat_id)
    if restored > 0:
        await update.message.reply_text(
            f"🧠 _Előző session visszatöltve: {restored // 2} kérdés-válasz pár_",
            parse_mode="Markdown"
        )
        log.info(f"[HISTORY] Visszatöltve: {restored} turn | chat: {chat_id}")
    await update.message.reply_text(
        "🤖 *AXON v8.4 ONLINE*\n\n"
        "📋 *Parancsok:*\n"
...
async def files_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    output_dir = OUTPUT_DIR  # v8.4: dinamikus path
...
async def run_simple_pipeline(
    ...
    # v8.4: History SQLite-ba mentés (csak history-t használó pipeline-oknál)
    if use_history:
...
async def run_developer_pipeline(
    ...
    # v8.4: History SQLite-ba mentés (bot restart után restore_history() visszatölti)
    persist_history(chat_id)
...
# UPLOADS_DIR moved to top of file (v8.4 – _BASE_DIR alapú)
    ...
    if _history_now:
        ...
        if _history_size > 6000:
            ...
            if not _compact_result.skipped:
                ...
                _clear_h(chat_id)
                for turn in _compact_result.compacted_history:
                    _add_h(chat_id, turn["role"], turn["content"],
                           turn.get("pipeline", "DEVELOPER"))
                await update.message.reply_text(
                    f"🗜 _Auto-compact: {_compact_result.removed_turns} turn → "
                    f"{_compact_result.summary_chars} kar összefoglaló_",
                    parse_mode="Markdown"
                )
                log.info(f"[AUTO-COMPACT] Kész: {_compact_result.removed_turns} turn tömörítve, {_compact_result.kept_turns} megmaradt")
...
def main():
    global watchman
    log.info("═══════════════════════════════════════════════════")
    log.info("  AXON TELEGRAM BRIDGE v8.4 – INDUL...")
    ...
    log.info(f"  Uploads dir: {UPLOADS_DIR}")
    ...
    async def post_init(application):
        ...
        # v8.4: Napi history cleanup – 7 napnál régebbi bejegyzések törlése
        async def history_cleanup_job(job_context):
            """Napi history cleanup."""
            deleted = cleanup_old_history()
            if deleted > 0:
                log.info(f"[CLEANUP] History cleanup: {deleted} sor törölve")

        application.job_queue.run_repeating(
            history_cleanup_job,
            interval=86400,   # naponta egyszer
            first=3600,       # 1 órával bot indítás után
            name="history_cleanup"
        )
        log.info("History cleanup job elindítva (naponta, 7 nap retention).")
